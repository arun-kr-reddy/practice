...
int main()
{
	try {
		...
		const uint32_t NumElements = src_pgm.width * src_pgm.height;
		const uint32_t BufferSize = NumElements * sizeof(int32_t);

		PGMData dst_pgm;
		dst_pgm.width = src_pgm.width;
		dst_pgm.height = src_pgm.height;
		dst_pgm.max_gray = 255;
		dst_pgm.ptr = (unsigned char *)malloc(NumElements);
		...
		int32_t *InBufferPtr = static_cast<int32_t *>(
		    Device.mapMemory(InBufferMemory, 0, BufferSize));
		for (int32_t I = 0; I < NumElements; ++I) {
		...
		if (std::ifstream ShaderFile{"Square.spv", std::ios::binary |
							       std::ios::ate}) {
		...
		vk::Pipeline ComputePipeline = Device.createComputePipeline(
		...
		vk::DescriptorBufferInfo InBufferInfo(
		    InBuffer, 0, NumElements * sizeof(int32_t));
		vk::DescriptorBufferInfo OutBufferInfo(
		    OutBuffer, 0, NumElements * sizeof(int32_t));
		...
		DestroyBuffer(B4);
#else
		InBufferPtr = static_cast<int32_t *>(
		    Device.mapMemory(InBufferMemory, 0, BufferSize));
		for (uint32_t I = 0; I < NumElements; ++I) {
			// std::cout << InBufferPtr[I] << " ";
		}
		...
		int32_t *OutBufferPtr = static_cast<int32_t *>(
		    Device.mapMemory(OutBufferMemory, 0, BufferSize));
		for (uint32_t I = 0; I < NumElements; ++I) {
			// std::cout << OutBufferPtr[I] << " ";
		}
		std::cout << std::endl;

...
int main()
{
    ...
    assert(err == CL_SUCCESS);
    dst1_buff = clCreateBuffer(context, CL_MEM_READ_WRITE, dst_pgm.width * dst_pgm.height * sizeof(uint8_t), NULL, &err);
    assert(err == CL_SUCCESS);
    dst2_buff = clCreateBuffer(context, CL_MEM_WRITE_ONLY, dst_pgm.width * dst_pgm.height * sizeof(uint8_t), NULL, &err);
    assert(err == CL_SUCCESS);
    ...
    err = clEnqueueNDRangeKernel(queue, kernel[0], 2, NULL, (const size_t *)&global_dim, NULL, 0, NULL, NULL);
    assert(err == CL_SUCCESS);

    err = clFinish(queue);
    assert(err == CL_SUCCESS);

    err = clEnqueueNDRangeKernel(queue, kernel[1], 2, NULL, (const size_t *)&global_dim, NULL, 0, NULL, NULL);
    assert(err == CL_SUCCESS);
    ...
    err = clEnqueueReadBuffer(queue, dst2_buff, CL_TRUE, 0, dst_pgm.width * dst_pgm.height * sizeof(uint8_t), dst_pgm.ptr, 0, NULL, &event);
    assert(err == CL_SUCCESS);

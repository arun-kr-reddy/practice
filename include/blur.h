#pragma once

#include "pgm.h"

void apply_blur(pgm_t *src_img, pgm_t *dst_img, uint8_t k_size) {
    int width = src_img->width;
    int height = src_img->height;
    uint8_t *src_buff = src_img->ptr;
    uint8_t *dst_buff = dst_img->ptr;
    uint8_t k_half_size = (k_size - 1) / 2;

    int8_t *kernel = malloc(k_size * k_size);
    int32_t kernel_sum = 0;
    for (uint32_t i = 0; i < (k_size * k_size); i++) {
        kernel[i] = 1;
        kernel_sum += kernel[i];
    }

    for (uint32_t y = 0; y < height; y++) {
        for (uint32_t x = 0; x < width; x++) {
            int16_t sum = 0;
            for (int k_y = -k_half_size; k_y <= k_half_size; k_y++) {
                for (int k_x = -k_half_size; k_x <= k_half_size; k_x++) {
                    int pos_y = y + k_y;
                    int pos_x = x + k_x;
                    // printf("%d %d %d %d\n", y, x, pos_y, pos_x);
                    if ((pos_x >= 0) && (pos_x < width) && (pos_y >= 0) &&
                        (pos_y < height)) {
                        sum += src_buff[pos_y * width + pos_x] *
                               kernel[(k_y + k_half_size) * k_size +
                                      (k_x + k_half_size)];
                    }
                }
            }
            dst_buff[y * width + x] = (uint8_t)(sum / kernel_sum);
        }
    }

    free(kernel);
}
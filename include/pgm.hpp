...
class pgm_t
{
  public:
    pgm_t (const std::string &filename)
    {
        ...
        char temp[3];
        fscanf (fp, "%s %d %d %d", temp, &this->_width, &this->_height,
                &this->_max_gray);
        assert (strcmp (temp, "P5") == 0);
        fgetc (fp);
        this->_ptr = reinterpret_cast<uint8_t *> (
            malloc (this->_height * this->_width));
